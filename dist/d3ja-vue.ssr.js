...
var interpolateTransformSvg = interpolateTransform(parseSvg, ", ", ")", ")");var frame = 0, // is an animation frame pending?
...
selection.prototype.transition = selection_transition;function defaultSeparation(a, b) {
...
var script = {
  ...
  methods: {
    getChart: function getChart() {
      ...
      var data = parseNewick(this.life);
      var width = 900;
      var outerRadius = width / 2;
      var innerRadius = outerRadius - 170;

      var legend = function legend(svg) {
        var g = svg.selectAll('g').data(color.domain()).join('g').attr('transform', function (d, i) {
          return "translate(".concat(-outerRadius, ",").concat(-outerRadius + i * 20, ")");
        });
      ...
      var color = ordinal().domain(['Bacteria', 'Eukaryota', 'Archaea']).range(category10);
      var cluster$1 = cluster().size([360, innerRadius]).separation(function (a, b) {
        return 1;
      });
      var root = hierarchy(data, function (d) {
        ...
        return a.value - b.value || ascending$1(a.data.length, b.data.length);
      });
      cluster$1(root);
      setRadius(root, root.data.length = 0, innerRadius / maxLength(root));
      setColor(root);
      var svg = select('.container').append('svg').attr('viewBox', [-outerRadius, -outerRadius, width, width]).attr('font-family', 'sans-serif').attr('font-size', 10);
      ...
      svg.append('g').selectAll('text').data(root.leaves()).join('text').attr('dy', '.31em').attr('transform', function (d) {
        return "rotate(".concat(d.x - 90, ") translate(").concat(innerRadius + 4, ",0)").concat(d.x < 180 ? '' : ' rotate(180)');
...
var __vue_module_identifier__ = "data-v-4452251a";
/* functional template */
...
var components=/*#__PURE__*/Object.freeze({__proto__:null,'default': install,TreeOfLife: __vue_component__});// only expose one global var, with component exports exposed as properties of
